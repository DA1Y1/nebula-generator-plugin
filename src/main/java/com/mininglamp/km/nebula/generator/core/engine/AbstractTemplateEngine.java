...
import com.mininglamp.km.nebula.generator.core.config.rules.FileType;
...
public abstract class AbstractTemplateEngine {
     ...
     * 生成一次性模板
     */
    private void disposableOutput(){
        PathTemplateConfig pathTemplateConfig = this.configBuilder.getPathTemplateConfig();
        if(pathTemplateConfig.isCreate()){
            try{
                Map<String, Object> objectMap = getGlobalObjectMap(pathTemplateConfig);
                for (CustomerTemplateInfo template : pathTemplateConfig.getTemplateInfo()){
                    String entityFile = String.format((template.getFilePath() + File.separator + "%s" + template.getSuffix()), template.getFileName());
                    writerFile(objectMap, templateFilePath(template.getTemplatePath()), entityFile);
                }
            } catch (Exception e) {
                e.printStackTrace();
            }
    ...
    public Map<String, Object> getGlobalObjectMap(PathTemplateConfig info){
        ...
        objectMap.put("path", config.getPathTemplateConfig());
        objectMap.put("idType", info.getIdType());
        objectMap.put("date", new SimpleDateFormat("yyyy-MM-dd").format(new Date()));
        for (CustomerTemplateInfo customer : info.getTemplateInfo()){
            objectMap.put(customer.getKey(), customer);
        }
        return objectMap;
    }
